...
<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;max-width:640px;margin:0 auto;color:#1e293b;line-height:1.65;background-color:#f1f5f9;padding:24px 16px;">
  <div style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(15,23,42,0.07);">
    ${headerHtml}
    <div style="padding:36px 32px 32px;background-color:#ffffff;">${bodyHtml}</div>
  </div>
...
function htmlHeader(title, subtitle) {
  const sub = subtitle
    ...
    : '';
  return `
<div style="background-color:#0f172a;padding:32px 32px 28px;text-align:center;">
...
function htmlInfoCard(rows) {
  const trs = rows.map(r =>
    `<tr>
      <td style="padding:9px 14px;color:#64748b;font-size:13px;width:150px;vertical-align:top;background-color:#f8fafc;">${r.label}</td>
      <td style="padding:9px 14px;font-size:14px;color:#1e293b;background-color:#f8fafc;">${r.value}</td>
    </tr>`
  ).join('');
  return `
...
<a href="${url}" style="text-decoration:none;display:block;margin-bottom:10px;">
  <div style="background-color:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px 18px;">
    <span style="font-weight:600;color:#1e293b;font-size:14px;">${title}</span>
    <span style="float:right;color:#64748b;font-size:18px;">&rarr;</span>
  </div>
</a>`;
}
